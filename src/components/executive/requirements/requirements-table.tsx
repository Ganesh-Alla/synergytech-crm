...
import { createRequirementsColumns } from './requirements-columns'
...
export function RequirementsTable() {
  ...
  const { requirements, requirementsLoading, hasLoaded } = useRequirementsStore()
  const { user } = useUserStore()
  const data = useMemo(() => requirements ?? [], [requirements])
  const columns = useMemo(() => createRequirementsColumns(user?.permission), [user?.permission])
  ...
  const globalFilterFn = (row: Row<Requirement>, _columnId: string, filterValue: string) => {
    if (!filterValue) return true

    const searchValue = filterValue.toLowerCase().trim()
    const requirement = row.original
    const fieldValue = requirement[searchField]
  ...
  // Handle row click
  const handleRowClick = (requirement: Requirement, event: React.MouseEvent<HTMLTableRowElement>) => {
  ...
  const getSearchPlaceholder = () => {
    const placeholders: Record<keyof Requirement, string> = {
      id: 'Search...',
      ...
      required_by_date: 'Search by required by date...',
      estimated_budget: 'Search by estimated budget...',
      estimated_budget_number: 'Search by estimated budget number...',
      currency_code: 'Search by currency code...',
      client_code: 'Search by client code...',
      assigned_to: 'Search by assigned to...',
      assigned_to_name: 'Search by assigned to name...',
      created_by: 'Search...',
      created_at: 'Search...',
      updated_at: 'Search...',
    }
...
if (hasLoaded && data.length === 0) {
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
        <div className="flex flex-1 items-center gap-2 min-w-0">
          <Select value={searchField} onValueChange={(value) => setSearchField(value as keyof Requirement)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="assigned_to_name">Assigned To Name</SelectItem>
            </SelectContent>
          </Select>
          <Input
            name='search'
        ...
        <div className="flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:gap-x-2 min-w-0">
          ...
          <div className='flex gap-x-2 flex-wrap'>
            <DataTableFacetedFilter
              ...
              title='Industry'
              options={requirementStatus.map((status) => ({ ...status }))}
            />
            <DataTableFacetedFilter
              column={table.getColumn('priority')}
              title='Priority'
              options={requirementPriority.map((priority) => ({ ...priority }))}
            />
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <div className="space-y-2 text-center mb-6">
            ...
            <p className="text-muted-foreground max-w-md text-sm">
              No requirements are available to display. Start by adding some requirements to get started.
            </p>
          </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs sm:text-sm text-muted-foreground">
                      ...
                      <span>Created: {formatDate(selectedRequirement.created_at)}</span>
                    </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-8 pb-6">
                ...
                <section>
                  ...
                  <div className="pl-0 sm:pl-1">
                    <div className="grid gap-1 sm:gap-2">
                      ...
                      <InfoRow icon={Mail} label="Description:">
                        <a
                          href={`mailto:${selectedRequirement.description}`}
                      ...
                      <InfoRow icon={Phone} label="Phone:">
                        {selectedRequirement.required_by_date ? (
                          <a
                            href={`tel:${selectedRequirement.required_by_date}`}
                      ...
                      <InfoRow icon={Building2} label="Client Code:">
                        {selectedRequirement.client_code}
                      </InfoRow>
                    </div>
                  </div>
                </section>
                ...
                <section>
                  <div className="flex items-center gap-2 mb-3 pb-2 border-b">
                    ...
                    <h2 className="text-lg sm:text-xl font-semibold">Requirement Information</h2>
                  </div>
                  ...
                  <div className="pl-0 sm:pl-1">
                    <div className="grid gap-1 sm:gap-2">
                      ...
                      {selectedRequirement.estimated_budget && (
                        <InfoRow icon={Globe} label="Website:">
                          <a
                            href={selectedRequirement.estimated_budget}
                            target="_blank"
                    ...
                    <div className="grid gap-1 sm:gap-2">
                      {selectedRequirement.required_by_date && (
                        <InfoRow icon={Calendar} label="Next Follow Up:">
                          {formatFollowUpDate(selectedRequirement.required_by_date)}
                        </InfoRow>
                      )}

                      {selectedRequirement.required_by_date && (
                        <InfoRow icon={Calendar} label="Required By Date:">
                          {formatDate(selectedRequirement.required_by_date)}
                        </InfoRow>
                      )}
                    </div>
                  </div>
