...
import { createExpenseColumns, expenseStatusOptions } from './expenses-columns'
...
export function ExpensesTable() {
  ...
  const { expenses, expensesLoading, hasLoaded } = useExpensesStore()
  const { user } = useUserStore()
  const data = useMemo(() => expenses ?? [], [expenses])
  const columns = useMemo(() => createExpenseColumns(user?.permission), [user?.permission])
  ...
  const [searchField, setSearchField] = useState<keyof Expense>("executive_id")
  ...
  const globalFilterFn = (row: Row<Expense>, _columnId: string, filterValue: string) => {
    if (!filterValue) return true
    
    const searchValue = filterValue.toLowerCase().trim()
    const expense = row.original
    const fieldValue = expense[searchField]
  ...
  const getSearchPlaceholder = () => {
    const placeholders: Record<keyof Expense, string> = {
      id: 'Search...',
      executive_id: 'Search by executive ID...',
      client_id: 'Search by client ID...',
      requirement_id: 'Search by requirement ID...',
      category_code: 'Search by category code...',
      category_id: 'Search by category ID...',
      amount: 'Search by amount...',
      currency_code: 'Search by currency code...',
      expense_date: 'Search by expense date...',
      merchant_name: 'Search by merchant name...',
      bill_number: 'Search by bill number...',
      notes: 'Search by notes...',
      receipt_url: 'Search by receipt URL...',
      status: 'Search by status...',
      approved_by: 'Search by approved by...',
      created_at: 'Search...',
      updated_at: 'Search...',
    }
...
if (hasLoaded && data.length === 0) {
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
        <div className="flex flex-1 items-center gap-2 min-w-0">
          <Select value={searchField} onValueChange={(value) => setSearchField(value as keyof Expense)}>
            <SelectTrigger className="h-8 w-[140px] shrink-0">
              <SelectValue placeholder="Search by" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="executive_id">Executive ID</SelectItem>
              <SelectItem value="client_id">Client ID</SelectItem>
              <SelectItem value="requirement_id">Requirement ID</SelectItem>
              <SelectItem value="category_code">Category Code</SelectItem>
              <SelectItem value="category_id">Category ID</SelectItem>
              ...
              <SelectItem value="receipt_url">Receipt URL</SelectItem>
        ...
        <div className="flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:gap-x-2 min-w-0">
          ...
          <div className='flex gap-x-2 flex-wrap'>
            <DataTableFacetedFilter
              ...
              title='Status'
              options={expenseStatusOptions.map((status: { label: string; value: string }) => ({ ...status }))}
            />
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <div className="space-y-2 text-center mb-6">
            ...
            <p className="text-muted-foreground max-w-md text-sm">
              No expenses are available to display. Start by adding some expenses to get started.
            </p>
          </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs sm:text-sm text-muted-foreground">
                      ...
                      <span>Created: {formatDate(selectedExpense.created_at)}</span>
                    </div>
                  </div>
                ...
                <section>
                  ...
                  <div className="pl-0 sm:pl-1">
                    <div className="grid gap-1 sm:gap-2">
                      ...
                      {selectedExpense.currency_code && (
                        <InfoRow icon={Globe} label="GST Number:">
                          <a
                            href={selectedExpense.currency_code}
                            target="_blank"
