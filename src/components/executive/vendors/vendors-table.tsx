...
import { createVendorsColumns, vendorStatusOptions } from './vendors-columns'
...
export function VendorsTable() {
  ...
  const { vendors, vendorsLoading, hasLoaded } = useVendorsStore()
  const { user } = useUserStore()
  const data = useMemo(() => vendors ?? [], [vendors])
  const columns = useMemo(() => createVendorsColumns(user?.permission), [user?.permission])
  ...
  const [searchField, setSearchField] = useState<keyof Vendor>("contact_name")
  ...
  const globalFilterFn = (row: Row<Vendor>, _columnId: string, filterValue: string) => {
  ...
  // Handle row click
  const handleRowClick = (vendor: Vendor, event: React.MouseEvent<HTMLTableRowElement>) => {
  ...
  const getSearchPlaceholder = () => {
    const placeholders: Record<keyof Vendor, string> = {
      id: 'Search...',
      vendor_code: 'Search by vendor code...',
      ...
      gst_number: 'Search by GST number...',
      address: 'Search by address...',
      payment_terms: 'Search by payment terms...',
...
if (hasLoaded && data.length === 0) {
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
        <div className="flex flex-1 items-center gap-2 min-w-0">
          <Select value={searchField} onValueChange={(value) => setSearchField(value as keyof Vendor)}>
        ...
        <div className="flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:gap-x-2 min-w-0">
          ...
          <div className='flex gap-x-2 flex-wrap'>
            <DataTableFacetedFilter
              ...
              title='Status'
              options={vendorStatusOptions.map((status: { label: string; value: string }) => ({ ...status }))}
            />
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <div className="space-y-2 text-center mb-6">
            ...
            <p className="text-muted-foreground max-w-md text-sm">
              No vendors are available to display. Start by adding some vendors to get started.
            </p>
          </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs sm:text-sm text-muted-foreground">
                      ...
                      <span>Created: {formatDate(selectedVendor.created_at)}</span>
                    </div>
                  </div>
                    ...
                    <div className="grid gap-1 sm:gap-2">
                      ...
                      {selectedVendor.gst_number && (
                        <InfoRow icon={Globe} label="GST Number:">
                          <a
                            href={selectedVendor.gst_number}
                            target="_blank"
