...
import { createSalesOrderColumns, salesOrderStatusOptions } from './sales-columns'
...
export function SalesOrderTable() {
  ...
  const { salesOrders, salesOrdersLoading, hasLoaded } = useSalesStore()
  const { user } = useUserStore()
  const data = useMemo(() => salesOrders ?? [], [salesOrders])
  const columns = useMemo(() => createSalesOrderColumns(user?.permission), [user?.permission])
  ...
  const [searchField, setSearchField] = useState<keyof SalesOrder>("order_number")
  ...
  const globalFilterFn = (row: Row<SalesOrder>, _columnId: string, filterValue: string) => {
    if (!filterValue) return true
    
    const searchValue = filterValue.toLowerCase().trim()
    const salesOrder = row.original
    const fieldValue = salesOrder[searchField]
  ...
  // Handle row click
  const handleRowClick = (salesOrder: SalesOrder, event: React.MouseEvent<HTMLTableRowElement>) => {
  ...
  const getSearchPlaceholder = () => {
    const placeholders: Record<keyof SalesOrder, string> = {
      id: 'Search...',
      client_id: 'Search by client ID...',
      requirement_id: 'Search by requirement ID...',
      synergy_quote_id: 'Search by Synergy Quote ID...',
      order_number: 'Search by order number...',
      order_date: 'Search by order date...',
      currency_code: 'Search by currency code...',
      total_cost: 'Search by total cost...',
...
if (salesOrdersLoading && !salesOrders) {
  return (
...
if (hasLoaded && data.length === 0) {
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
        <div className="flex flex-1 items-center gap-2 min-w-0">
          <Select value={searchField} onValueChange={(value) => setSearchField(value as keyof SalesOrder)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="synergy_quote_id">Synergy Quote ID</SelectItem>
        ...
        <div className="flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:gap-x-2 min-w-0">
          ...
          <div className='flex gap-x-2 flex-wrap'>
            <DataTableFacetedFilter
              ...
              title='Status'
              options={salesOrderStatusOptions.map((status: { label: string; value: string }) => ({ ...status }))}
            />
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <div className="space-y-2 text-center mb-6">
            ...
            <p className="text-muted-foreground max-w-md text-sm">
              No sales orders are available to display. Start by adding some sales orders to get started.
            </p>
          </div>
            ...
            <>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs sm:text-sm text-muted-foreground">
                      ...
                      <span>Created: {formatDate(selectedSalesOrder.created_at)}</span>
                    </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-8 pb-6">
                ...
                <section>
                  ...
                  <div className="pl-0 sm:pl-1">
                    <div className="grid gap-1 sm:gap-2">
                      ...
                      <InfoRow icon={Mail} label="Order Number:">
                        <a
                          href={`mailto:${selectedSalesOrder.order_number}`}
                      ...
                      <InfoRow icon={Phone} label="Order Date:">
                        {selectedSalesOrder.order_date ? (
                          <a
                            href={`tel:${selectedSalesOrder.order_date}`}
                ...
                <section>
                  ...
                  <div className="pl-0 sm:pl-1">
                    <div className="grid gap-1 sm:gap-2">
                      ...
                      {selectedSalesOrder.currency_code && (
                        <InfoRow icon={Globe} label="GST Number:">
                          <a
                            href={selectedSalesOrder.currency_code}
                            target="_blank"
                ...
                <section>
                  ...
                  <div className="pl-0 sm:pl-1">
                    <div className="grid gap-1 sm:gap-2">
                      {selectedSalesOrder.created_at && (
                        <InfoRow icon={Calendar} label="Created At:">
                          {formatDate(selectedSalesOrder.created_at)}
                        </InfoRow>
                      )}

                      {selectedSalesOrder.updated_at && (
                        <InfoRow icon={Calendar} label="Updated At:">
                          {formatDate(selectedSalesOrder.updated_at)}
                        </InfoRow>
                      )}
                    </div>
                  </div>
